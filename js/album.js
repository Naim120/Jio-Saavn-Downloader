window.onload = async() => {
	...
	if(param) {
		...
		var get_data = await (await fetch(`${API_CONFIG.url}/album/?query=${param}`)).json()
		if(get_data.result === 'false') {
			...
			get_data.songs.forEach(song => {
				txt += `<tr><td><a href='/song/?query=${song.perma_url}'>${song.song}</a></td><td>${song.primary_artists}</td></tr>`;
			});
			txt += "</tbody></table>"
